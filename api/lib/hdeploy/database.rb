  ...
  class Database
    ...
    def initialize
      ...
      @queries = {
        ...
        put_artifact:    'REPLACE INTO artifacts (artifact,app,source) VALUES(?,?,?)',
        ...
        get_distribute_env: 'SELECT distribute.artifact,source FROM artifacts INNER JOIN distribute ON distribute.artifact = artifacts.artifact WHERE artifacts.app = ? AND env = ?',
        ...
        get_artifact_list:    'SELECT artifact,source FROM artifacts WHERE app = ?',
        get_artifact:         'SELECT artifact,source FROM artifacts WHERE app = ? AND artifact = ?',
