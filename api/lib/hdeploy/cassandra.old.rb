...
module HDeploy
  class Cassandra
    ...
    def initialize
      ...
      @indexes = [
        "app_artifacts on artifacts (app)",
        "app_distribute on distribute (app)",
      ]
    end
    ...
    def _cass_create_tables()
      ...
      @indexes.each do |i|
        @cass.execute("CREATE INDEX IF NOT EXISTS #{i}")
      end
    end
    ...
    def method_missing(method, *args, &block)
      ...
      if method == :prepare or method == :execute
        ...
        begin
          ...
          elsif e.to_s =~ /Cannot execute this query as it might involve data filtering/
            # Indexes were not created - just create all indexes
            puts "Creating indexes!"
            @indexes.each do |i|
              puts "CREATE INDEX IF NOT EXISTS #{i}"
              @cass.execute("CREATE INDEX IF NOT EXISTS #{i}")
            end
            r = @cass.send(method, *args, &block)
          else
