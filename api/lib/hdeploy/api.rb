...
module HDeploy
  class API < Sinatra::Base
    ...
    def initialize
      ...
      @conf = HDeploy::Conf.instance #FIXME: configure something else than local cassandra - and catch errors!

      @conf['api'] or raise "no api section in configuration"

      # Not very elegant but it works...
      if ENV['HDEPLOY_API_NAME']
        @conf['api'].has_key?(ENV['HDEPLOY_API_NAME']) or raise "no '#{ENV['HDEPLOY_API_NAME']}' section in api configuration (was specified by env HDEPLOY_API_NAME)"
        @conf_api = @conf['api'][ENV['HDEPLOY_API_NAME']]
      else
        @conf['api'].has_key?('default') or raise "no 'default' section in api configuration (to choose another section please set env HDEPLOY_API_NAME)"
        @conf_api = @conf['api']['default']
      end
    ...
    def authorized?
      ...
      @auth.provided? and @auth.basic? and @auth.credentials and @auth.credentials == @conf_api.values_at('user','pass')
    end
    ...
    get '/distribute/:app' do |app|
      ...
      env_exists = {}
      @cass.execute("SELECT env FROM active_env WHERE app = '#{app}' ALLOW FILTERING").each do |row|
